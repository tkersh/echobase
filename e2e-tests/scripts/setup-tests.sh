...
if [ ! -f ".env" ]; then
    ...
    echo ""
    echo "This file is tracked in git and should exist after cloning."
    echo "Try: git checkout .env"
    echo ""
    exit 1
fi
...
# Export database credentials to e2e-tests/.env.secrets (not .env, which is tracked in git)
# Note: We're already in e2e-tests directory from the previous step
echo ""
echo "Exporting test configuration..."

# Write secrets to .env.secrets (gitignored, regenerated each run)
cat > .env.secrets <<EOF
# Auto-generated by setup-tests.sh — do not commit
DB_PASSWORD=${MYSQL_PASSWORD}
EOF

# Update non-secret values in .env
# Only rewrite if something has actually changed
NEEDS_UPDATE=false
grep -q "^DB_USER=${MYSQL_USER}$" .env 2>/dev/null || NEEDS_UPDATE=true
grep -q "^WEB_BASE_URL=${WEB_URL}$" .env 2>/dev/null || NEEDS_UPDATE=true

if [ "$NEEDS_UPDATE" = true ]; then
    # Remove old auto-generated values and deprecated FRONTEND_BASE_URL
    grep -v "^DB_USER=" .env | grep -v "^WEB_BASE_URL=" | grep -v "^FRONTEND_BASE_URL=" > .env.tmp
    mv .env.tmp .env
    echo "" >> .env
    echo "# Auto-generated by setup-tests.sh" >> .env
    echo "DB_USER=${MYSQL_USER}" >> .env
    echo "WEB_BASE_URL=${WEB_URL}" >> .env
fi

print_success "Test secrets exported to e2e-tests/.env.secrets"
cd ..
